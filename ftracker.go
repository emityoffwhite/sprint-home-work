...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Ходьба":
		distance := distance(action)                                          // вызовите здесь необходимую функцию
		speed := meanSpeed(action, duration)                                  // вызовите здесь необходимую функцию
		calories, _ := WalkingSpentCalories(action, duration, weight, height) // вызовите здесь необходимую функцию
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, distance, speed, calories)
...
func WalkingSpentCalories(action int, duration, weight, height float64) (float64, float64) {
	speed := meanSpeed(action, duration)
	speedMs := speed * kmhInMsec
	heightM := height / cmInM

	return (walkingCaloriesWeightMultiplier*weight +
		(math.Pow(speedMs, 2)/heightM)*walkingSpeedHeightMultiplier*weight) *
		duration * minInH, 0
